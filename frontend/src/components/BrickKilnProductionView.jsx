...
export default function BrickKilnProductionView({ firm }) {
  ...
  const handleSavePathai = (e) => {
    ...
    try {
      ...
      alert(`✓ Pathai entry saved! ${rawBricksCount} Raw Bricks created (${unitWeightKg} kg/brick). Auto-deducted ${estimatedSoilTons} Tons Mud stock. Reflected in Master Inventory!`);
      setRawBricksCount('');
      loadData();
    } catch (err) {
      alert(err.message);
