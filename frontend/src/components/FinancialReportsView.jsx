...
export default function FinancialReportsView({ firm }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('vouchers_updated', loadReports);
    window.addEventListener('stock_updated', loadReports);
    return () => {
      window.removeEventListener('storage', loadReports);
      window.removeEventListener('vouchers_updated', loadReports);
      window.removeEventListener('stock_updated', loadReports);
    };
  }, [firm, activeFirmId]);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '16px' }}>
      ...
      <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '12px', border: '1px solid #cbd5e1', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
          <span>Purchases & Material Inward (-)</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0 0 0', marginTop: '10px', borderTop: '2px dashed #cbd5e1', fontSize: '15px' }}>
          ...
          <strong style={{ color: tradingAndPL.netProfit >= 0 ? '#059669' : '#dc2626' }}>
