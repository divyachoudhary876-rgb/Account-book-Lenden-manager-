...
export default function InventoryStockView({ firm }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('vouchers_updated', loadStockData);
    return () => {
      window.removeEventListener('storage', loadStockData);
      window.removeEventListener('stock_updated', loadStockData);
      window.removeEventListener('vouchers_updated', loadStockData);
    };
  }, [firm, activeFirmId]);
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '12px', border: '1px solid #cbd5e1' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
        <div>
          <h3 style={{ margin: 0, color: '#0f172a', fontSize: '18px' }}>📦 Real-Time Stock & Inventory</h3>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', minWidth: '500px' }}>
          <thead>
            <tr style={{ backgroundColor: '#0f172a', color: '#ffffff' }}>
              <th style={{ padding: '10px', textAlign: 'left' }}>Item / Material</th>
              <th style={{ padding: '10px', textAlign: 'center' }}>Unit</th>
              <th style={{ padding: '10px', textAlign: 'right' }}>Current Stock</th>
              <th style={{ padding: '10px', textAlign: 'right' }}>Avg Cost Rate (₹)</th>
            ...
            {stockList.length === 0 ? (
              <tr>
                <td colSpan="5" style={{ textAlign: 'center', padding: '20px', color: '#94a3b8' }}>
                  No stock entries found. Purchase entry post karein.
                </td>
              </tr>
            ) : (
              stockList.map(item => {
                ...
                return (
                  <tr key={item.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: '10px', fontWeight: 'bold' }}>{item.item_name}</td>
                    <td style={{ padding: '10px', textAlign: 'center' }}>{item.unit || 'Units'}</td>
                    <td style={{ padding: '10px', textAlign: 'right', fontWeight: 'bold', color: stock > 0 ? '#059669' : '#dc2626' }}>
                      {stock.toFixed(2)}
                    </td>
                    <td style={{ padding: '10px', textAlign: 'right' }}>₹{rate.toFixed(2)}</td>
                    <td style={{ padding: '10px', textAlign: 'right', fontWeight: 'bold', color: '#1e3a8a' }}>
                      ₹{(stock * rate).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                    </td>
                  </tr>
                );
              })
