...
export default function VoucherEntryForm({ firm }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!drAccount || !crAccount) {
      alert("⚠️ Select Accounts from dropdowns first!");
      return;
    }
    try {
      const payload = {
      ...
      alert(`✓ Voucher ${created.id} Posted Successfully for Date: ${created.voucher_date}`);
      setAmount('');
      setNarration('');
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '12px', border: '1px solid #cbd5e1', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
      <h3 style={{ margin: '0 0 16px 0', color: '#0f172a', fontSize: '18px' }}>📒 Double-Entry Voucher Posting</h3>

      <form onSubmit={handleSubmit} style={{ backgroundColor: '#f8fafc', padding: '14px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
        
        {/* Row 1: Date & Voucher Type Picker */}
        ...
        {/* Row 2: Debit & Credit Accounts Dropdowns */}
