...
import { postDoubleEntryVoucher, getAccountCurrentBalance } from '../utils/voucherPostingEngine.js';
...
export default function VoucherEntryForm({ firm }) {
  ...
  const handlePostVoucher = (e) => {
    ...
    try {
      ...
      alert(`✓ ${voucherType} Voucher of ₹${amount} posted successfully! Balance Sheet, Day Book, and Ledger Milan updated.`);
      setAmount('');
      setNarration('');
    } catch (err) {
      alert(err.message);
  ...
  const creditCashBalance = creditAcc ? getAccountCurrentBalance(activeFirmId, creditAcc) : 0;
      ...
      <form onSubmit={handlePostVoucher} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          ...
          <div>
            ...
            <span style={{ fontSize: '10px', color: creditCashBalance < 0 ? '#dc2626' : '#059669', fontWeight: 'bold' }}>
              Avail Balance: ₹{creditCashBalance.toFixed(2)}
            </span>
          </div>
        </div>
